...
literalValueBufferS = structure["LiteralValueBuffer"]
objKeyBufferS = structure["ObjKeyBuffer"]
objShapeTableS = structure["ObjShapeTable"]
...
def parse(f):
    ...
    for i in range(header["functionCount"]):
        ...
        if (functionHeader["flags"] >> 5) & 1:
            ...
            saved_pos = f.tell()
            large_offset = (functionHeader["functionName"] << 16) | functionHeader[
                "offset"
            ]
            f.seek(large_offset)
    ...
    literalValueBufferS[2] = header["literalValueBufferSize"]
    ...
    for _ in range(header["objShapeTableCount"]):
        objShapeTableEntry = {}
        for key in objShapeTableS:
            objShapeTableEntry[key] = read(f, objShapeTableS[key])
        objShapeTable.append(objShapeTableEntry)
...
def export(obj, f):
    ...
    literalValueBufferS[2] = header["literalValueBufferSize"]
    write(f, literalValueBuffer, literalValueBufferS)
    ...
    for i in range(header["objShapeTableCount"]):
        objShapeTableEntry = objShapeTable[i]
        for key in objShapeTableS:
            write(f, objShapeTableEntry[key], objShapeTableS[key])
    ...
    for overflowedFunctionHeader in overflowedFunctionHeaders:
        ...
        large_offset = (
            smallFunctionHeader["functionName"] << 16
        ) | smallFunctionHeader["offset"]
        f.seek(large_offset)
        for key in functionHeaderS:
