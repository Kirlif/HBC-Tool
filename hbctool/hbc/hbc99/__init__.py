...
ValueNullOrKeyPrivateNameTag = 0
TrueTag = 1 << 4
FalseTag = 2 << 4
NumberTag = 3 << 4
LongStringTag = 4 << 4
...
class HBC99:
    ...
    def getFunction(self, fid, disasm=True):
        ...
        numberRegCount = functionHeader["numberRegCount"]
        nonPtrRegCount = functionHeader["nonPtrRegCount"]
    ...
    def setFunction(self, fid, func, disasm=True):
        ...
        functionHeader["numberRegCount"] = numberRegCount
        functionHeader["nonPtrRegCount"] = nonPtrRegCount
    ...
    def _SLPToString(self, tag, buf, iid, ind):
        ...
        if tag == UndefinedTag:
            ...
            val = unpack("<L", bytes(buf[start : start + 4]))[0]
            ind += 4
        elif tag == ValueNullOrKeyPrivateNameTag:
            type = "Null"
            val = None
        elif tag == TrueTag:
            type = "Boolean"
    ...
    def getLiteralValueBuffer(self, aid):
        ...
        for _ in range(tag[0]):
            t, val, ind = self._SLPToString(
                tag[1], self.getObj()["literalValueBuffer"], aid, ind
            )
            arr.append(val)
    ...
    def getObjectShapeTable(self, vid):
        assert vid >= 0 and vid < self.getObjShapeTableCount(), "Invalid ObjValue ID"
        tag = self._checkBufferTag(self.getObj()["objShapeTable"], vid)
        ind = 2 if tag[0] > 0x0F else 1
        keys = []
        t = None
        for _ in range(tag[0]):
            t, val, ind = self._SLPToString(
                tag[1], self.getObj()["objShapeTable"], vid, ind
            )
            keys.append(val)
